...
**حالةٌ خاصّةٌ مهمّة — الفصل بصيغة أرقامٍ مثل «١/٣» أو «1/3» أو «٢-٤»:**
ضع النصّ **كاملاً كما هو** في new_grade، و**اترك new_section فارغة**.
لا تقسمه بنفسك ولا تحدّد أيّهما الصف وأيّهما الشعبة — التطبيق يقسمه
بقاعدةٍ ثابتة. وكل محاولةٍ لقسمته هنا كانت تعكس الترتيب فتُعطي المعلّم
صفّاً غير صفّه.
...
Deno.serve(async (req) => {
    ...
    if (!quota || quota.allowed !== true) {
        ...
        const cap = byPages ? (quota?.pages_limit ?? 700) : (quota?.quota_limit ?? 100);
        return json({
            error: byPages
                ? 'بلغتَ حدَّ الصفحات لهذا العام الدراسيّ (' + cap + ' صفحة). '
                  + 'يتجدّد مع العام القادم — وحتى ذلك الحين أدخل الأسماء والجدول يدوياً.'
                : 'بلغتَ حدَّ الاستيراد لهذا العام الدراسيّ (' + cap + ' عمليّة). '
                  + 'يتجدّد مع العام القادم — وحتى ذلك الحين أدخل الأسماء والجدول يدوياً.',
            code:  'quota_exceeded',
