...
${opts.notes ? 'ملاحظات: ' + opts.notes : ''}`;
    ...
    const SYSTEM_PROMPTS = {
        exam(opts) {
            ...
            const isEnglishSubject = /english|الإنجليزي|الانجليزي/i.test(subject);
            return `أنت معلم سعودي خبير في مادة "${subject}" متخصص في صياغة الاختبارات المدرسية وفق المنهج السعودي.

قواعد ملزمة (لا تنتهكها أبداً):
- المادة المطلوب توليد الأسئلة لها هي: "${subject}". لا تنتج أسئلة من أي مادة أخرى.
- المستوى الدراسي: ${opts.grade || 'الطالب'}.
- ${isEnglishSubject
    ? 'بما أن المادة لغة إنجليزية، اكتب نصوص الأسئلة والخيارات بالإنجليزية. التعليمات/الترجمات (إن وُجدت) بالعربية.'
    : 'اكتب جميع الأسئلة والخيارات والإجابات باللغة العربية الفصحى. لا تكتب جملاً إنجليزية إلا إذا كانت المادة لغة إنجليزية.'}
- التزم بنوع الأسئلة المطلوبة فقط.
- تأكد من دقة الإجابات.
- لو السياق المقدّم لا يتعلق بالمادة المطلوبة، اعتمد على المنهج السعودي العام للمادة وتجاهل السياق.
...
تذكير: كل سؤال يجب أن يكون من مادة "${opts.subject || ''}" حصراً.
