...
(function (global) {
    ...
    /* `Words.count` لا عدٌّ خام: فيها تمييزُ العربيّة («طالبان» · «٣ طلاب»
       ...
       وموضعُها الآن النخبةُ بعد الربط وحدَها: العددُ سقط من السؤال بنصّه
       الذي كتبه (٧ سبتمبر)، فيراه بعد أن يوافق لا قبله. */
    ...
    async function offerSharedRoster(cls) {
        ...
        /* ══ سؤالٌ بلا شرح — ونصُّه بحرفه من كلامه (٧ سبتمبر) ══
           ...
           و«استخدام» لا «نسخ» — بقراره بعد التنبيه. والفرقُ ليس لفظاً:
           الكشفُ **واحدٌ يقرؤه الفصلان**، فتصحيحُ اسمٍ في أحدهما يظهر في
           الآخر. و«نسخ» توهم قائمتين مستقلّتين، فيُفاجأ المعلّمُ حين
           يعدّل — وهو الذي رفضه يوم اختار «ب» («نسختان تفترقان»).
           فلا تُعاد الكلمةُ إلى «نسخ»، ولا تُحوَّل الشيفرةُ إلى نسخٍ.

           ⚠️ وسقط العددُ من السؤال بنصّه هذا، وكان هو ما يكشف المطابقةَ
           الخاطئة قبل الموافقة. صار يُرى في النخبة بعدها. */
        const ok = await global.TeacherApp.confirm({
            title:   'نفس الأسماء؟',
            message: 'عندك «' + name + '» بنفس الاسم. '
                   + 'هل تريد استخدام نفس أسماء الطلاب؟',
            ok:     'نعم',
            cancel: 'لا، سأكتبهم بنفسي',
