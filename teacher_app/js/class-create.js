...
(function (global) {
    ...
    async function offerSharedRoster(cls) {
        ...
        /* ══ سؤالٌ بلا شرح — ونصُّه بحرفه من كلامه (٧ سبتمبر) ══
           ...
           ⚠️ و«نسخ» كلمتُه، والفعلُ **ربطٌ لا نسخ**: الكشفُ واحدٌ يقرؤه
           الفصلان، فتصحيحُ اسمٍ في أحدهما يظهر في الآخر — وهو الذي أراده
           حين اختار «ب» ورفض النسخ («نسختان تفترقان»). فلا تُحوَّل
           الشيفرةُ إلى نسخٍ لأجل الكلمة.
        ...
        const ok = await global.TeacherApp.confirm({
            ...
            message: 'عندك «' + name + '» بنفس الاسم. '
                   + 'هل تريد نسخ أسماء الطلاب مباشرة؟',
            ok:     'نعم',
