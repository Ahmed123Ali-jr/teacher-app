...
(function (global) {
    ...
    async function replaceWeekly(items) {
        ...
        const notifications = items.map((it) => ({
            ...
            body: it.body,
            sound: it.sound,
            /* **يخترق وضعَ التركيز** — بقرار المعلّم (٣ سبتمبر ٢٠٢٦). ومعلّمٌ
               يشغّل «تركيز» أثناء الحصّة كان لا يصله تنبيهُ التي بعدها البتّة،
               وهو الحالُ الغالبُ لا النادر.
               وثلاثةُ شروطٍ لا يعمل بدونها، تُراجع يومَ التغليف:
                 ١) `@capacitor/local-notifications` **٨٫١٫٠ فأعلى** — قبله
                    لا وجودَ للحقل أصلاً، ويُتجاهل بصمت.
                 ٢) قدرةُ **Time Sensitive Notifications** تُفعَّل في Xcode
                    (Signing & Capabilities). لا تحتاج إذناً من آبل، لكنّها
                    بدونها لا تخترق شيئاً.
                 ٣) وللمعلّم أن يُطفئها لكلّ وضعِ تركيزٍ من إعدادات جهازه —
                    وهذا حقُّه، ولا يُقاوَم.
               وهو **غيرُ** مفتاح الصامت: لا `Critical Alerts`. */
