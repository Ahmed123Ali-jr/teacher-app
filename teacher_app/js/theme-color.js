...
(function (global) {
    ...
    /**
     * الحدُّ الأعلى للإضاءة عند صبغةٍ وإشباع — **ثلاثةُ قيود**:
     ...
     * ٣) **والذهبيُّ الفاتحُ ‎#D6B96F‎ فوق الدرجة الوسطى عند ‎٤٫٥:١‎** —
     *    أُضيف في ٤ سبتمبر ٢٠٢٦ بعد أن كشفه فحصُ `contrast.html` بلونِ
     *    المعلّم: بطاقةُ الهُويّة (`.idc`) تبدأ تدرّجُها من `--primary-mid`
     *    وحبرُها ذهبيٌّ **أفتحُ** من ‎#C9A961‎ — فطرفان يرتفعان معاً ويسقط
     *    التباينُ إلى ‎4.37‎ عند أفتحِ أسودَ يُتاح (‎#404040‎). والقيدُ
     *    رخيص: **عشرُ درجاتٍ من ٢١٨** تخرج من المنزلق، والبتروليُّ لا يتغيّر.
     *    والعتبةُ هنا ‎4.56‎ لا ‎4.5‎: `maxLightness` تحسب من ‎(h,s,l)‎ مباشرةً،
     *    و`scale` تحسب من اللون بعد تحويله إلى ‎#RRGGBB‎ ثمّ رجوعه — ورحلةُ
     *    الذهاب والإياب تُضيّع كسوراً، فينزل المقيسُ إلى ‎4.48‎ عند لونٍ
     *    أجازته الحلقةُ عند ‎4.50‎. والهامشُ يبتلع الفرق (قِيس: صفرٌ تحت
     *    العتبة بعده، وكان واحداً).
    ...
    function maxLightness(h, s) {
        ...
        for (let l = 4; l <= 60; l += 1) {
            const ok = ratio('#FFFFFF', step(h, s, l, 'light')) >= 7
                    ...
                    && ratio('#D6B96F', step(h, s, l, 'mid')) >= 4.56;
    ...
    /** ونظيرتُها صعوداً — للوضع الداكن. */
    function lightenUntil(h, s, l, bed, need) {
        let x = l;
        let hex = hslToHex(h, s, x);
        while (x < 96 && ratio(hex, bed) < need) { x += 1; hex = hslToHex(h, s, x); }
        return hex;
    }

    /** أرضيّةُ الوضع الداكن (`theme-dark.css: --bg`). */
    ...
    function scale(hex, dark) {
        ...
        /* ── و‎+21.5‎ وحدَها لا تكفي لكلّ لون ──
           كانت رفعةً ثابتةً مقيسةً من البترولي: ‎#0A3F4A‎ ← ‎#1C8EA6‎ = ‎4.93‎
           على أرضيّة الداكن. لكنّ المعلّم يختار من ‎218‎ لوناً، ومنها ما
           يبدأ من ‎L=10‎ — فتنزل الرفعةُ الثابتة إلى ‎1.64‎ (نيليّ ‎#100E25‎)،
           وهي حدُّ تحديدٍ وحلقةُ تركيزٍ على أرضيّةٍ سوداء، وعتبتُها ‎٣:١‎ لغير
           النصّ. قِيس على الألوان كلِّها (٤ سبتمبر ٢٠٢٦).

           فصارت الرفعةُ **مشروطةً لا ثابتة**: تبدأ من ‎+21.5‎ كما كانت — فلا
           يتغيّر شيءٌ للبترولي ولا لما يبلغ العتبةَ بها — ثمّ تُرفع درجةً
           درجةً حتى تبلغ ‎3.2‎ (هامشٌ فوق ‎٣‎ يحمل تقريبَ التحويل). */
        const light = dark
            ? lightenUntil(h, clamp(s - 5, 0, 100),
                           clamp(l + 21.5, 0, 100), DARK_BG, 3.2)
            : step(h, s, l, 'light');
        return {
            '--pf-royal':       hex,
