...
(function (global) {
    ...
    async function render(container) {
        ...
        /* ── زرٌّ واحدٌ عريضٌ في أسفل الشاشة ──
           ...
           وصنفُه `start-cta` لا `btn btn-primary` — بقرار المعلّم
           (٢٨ أغسطس ٢٠٢٦): هو والزرُّ الذي في الرئيسية يقولان الشيءَ نفسَه
           ويقعان في الموضع نفسِه من الشاشة، فيجب أن يُريا واحداً.
           و`btn-primary` مسطّحةٌ بظلٍّ عامّ، و`start-cta` لها لمعةٌ داخليّةٌ
           في أعلاها وظلٌّ بلون الهُويّة تحتها فتُقرأ مجسَّمة.
           ولم يُنسخ التنسيق: ستُّ شاشاتٍ أخرى تستعير `start-cta` للسبب
           نفسِه (انظر class.js:902).

           ولا يُكتب شيءٌ من هذا داخل النصّ القالبيّ أدناه: العلامةُ
           المائلة تُنهي السلسلة. وقعتُ فيها مرّتين. */
        container.innerHTML = `
            ...
            <div class="classes-fab">
                <button type="button" class="start-cta" id="btn-add-class">+ إضافة فصل جديد</button>
            </div>
        `;
