...
(function (global) {
    ...
    function restCardHtml(kind, off) {
        if (kind === 'weekend') {
            /* ══ معاينةُ خطِّ التحيّة (?rest=1) — تنتظر قرارَه ══
               قال (٣ سبتمبر ٢٠٢٦): «غيّر نوع الخطّ… وغيّر الجملة إلى إجازة
               نهاية أسبوع سعيدة». والمقولاتُ سقطت بقوله «لا تعتمد».

               وثلاثةُ خطوطٍ تُعرض معاً ليختار بحرفٍ واحد. وكلُّها **من
               خطوط النظام في أجهزة آبل** — لا ملفَّ يُحمَّل ولا حجمَ يُزاد
               ولا شيءَ يُطلب من الشبكة. ولكلٍّ بديلٌ ينتهي إلى خطّ التطبيق
               نفسِه، فمن لا يملكها يرى الحاليَّ لا خطّاً غريباً. */
            if (document.body.classList.contains('rest-quote')) {
                const TRY = [
                    { k: 'أ', cls: 'f-plex',    name: 'خطّ التطبيق — وزنٌ أخفّ' },
                    { k: 'ب', cls: 'f-bayan',   name: 'نسخيٌّ (Al Bayan)' },
                    { k: 'ج', cls: 'f-damascus', name: 'دمشق (Damascus)' }
                ];
                return `
                    <div class="home-hero-alt rest-q">
                        ${TRY.map((f) => `
                            <div class="rq-try">
                                <div class="rq-t ${f.cls}">إجازة نهاية أسبوع سعيدة</div>
                                <div class="rq-tag">${f.k} · ${f.name}</div>
                            </div>`).join('')}
