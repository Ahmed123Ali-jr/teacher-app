...
(function (global) {
    ...
    /* أول فتح بلا فصول: بنيةُ «بقي الجدول» نفسُها — لافتةٌ أعلى الشاشة
       وزرّان أسفلَها. بقرار المعلّم (٢٨ أغسطس ٢٠٢٦): «أبيها تكون نفس
       تصميم الصفحة الرئيسية لما يكون عندي فصول ولم أنشئ الجدول».

       وكانت بطاقتين متساويتين بسهمين في صدر الشاشة، وتحتهما ‎800‎ بكسلٍ
       فارغة — والزرّان بعيدان عن الإبهام. فصارت الحالتان الأوليان بشكلٍ
       واحد: المعلّمُ يمرّ بهما متتابعتين في أوّل دقيقةٍ من عمر حسابه،
       فاختلافُ الشكل بينهما يُقرأ انتقالاً إلى مكانٍ آخر لا تقدّماً في
       المكان نفسه.

       و«ارفع جدولك» يبقى الطريقَ الأوّل بقرار المستخدم (١٧ أغسطس ٢٠٢٦):
       هو الذي يبني الفصولَ والحصصَ معاً، فالبدءُ به أسرعُ من إضافة ستّة
       فصولٍ بيده. والإضافةُ اليدوية تبقى ظاهرةً لمن ليس جدولُه بيده.

       واللافتةُ تقول ما ينقص لا ما يفعله الزرّ — وهو الدرسُ نفسُه المسجَّل
       في `startScheduleHtml` أدناه. */
    function startFirstClassHtml() {
        return `
            ...
            <a href="#/schedule?import=1" class="start-cta">+ ارفع جدولك الأسبوعي</a>
            <button type="button" class="start-add-class" data-add-class>+ إضافة فصل</button>`;
    }

    /* فصول موجودة بلا جدول: لافتةُ «بقي الجدول» أعلى الشاشة، والزرّان أسفلها.
       تصميمُ «و» — اختاره المعلّم من ستّةٍ عُرضت عليه (١٩ أغسطس ٢٠٢٦).

       وكان قبلها صندوقاً فيه حلقةٌ ورمزُ تقويمٍ وعنوانٌ يعيد نصَّ الزرّ
       حرفاً بحرف، فيقرأ المعلّمُ الجملةَ مرّتين ويضغط الأولى فلا تستجيب.
       ثمّ بقي الزرُّ وحدَه فصار تحته ‎266‎ بكسلاً فارغة. فحلّت اللافتةُ محلَّ
       الحلقة. ولا رمزَ تعبيريّاً فيها.

       والشرحُ يذكر الطريقين صراحةً بقرار المعلّم (٢٨ أغسطس ٢٠٢٦) — وهو
       عدولٌ عن قاعدة «تقول ما ينقص لا ما يفعله الزرّ» المكتوبة أعلاه،
       وله وجه: الزرُّ يقول «أضف جدولك» فيظنُّ المعلّمُ أنّ أمامه سبعاً
       وثلاثين خانةً يملؤها بيده، ولا يعلم أنّ صورةً واحدةً تكفي. فذكرُ
       الرفع هنا يوفّر عليه الطريق.

       ووجهةُ الزرّ صارت `?import=1` لا `#/schedule` وحدَه: زرُّ الاستيراد
       في شاشة الجدول لا يُرسم إلا في وضع التعديل (schedule.js:208)، فمن
       وصل إليها ساكنةً وقف أمام جدولٍ فارغٍ يبحث عن الرفع الذي وُعد به.
       والعلامةُ تفتح صندوق الاستيراد رأساً، وإلغاؤه يترك الشاشةَ في وضع
       التعديل — فالطريقان اللذان تذكرهما اللافتةُ كلاهما في متناوله.

       ولا يُكتب شيءٌ من هذا داخل النصّ القالبيّ أدناه: العلامةُ المائلة
       تُنهي السلسلة. جُرّب فسقطت الشاشةُ كلُّها بيضاء، و`node --check`
       مرّ عليها — فهو يفحص التركيب لا المعنى. */
    function startScheduleHtml() {
        return `
            <div class="start-note">
                <b>لا يوجد لديك جدول</b>
                <span>ارفع صورةً أو ملفاً لجدولك فيُنشأ مباشرةً، أو أضفه يدوياً</span>
            </div>
            <div class="start-gap"></div>
            <a href="#/schedule?import=1" class="start-cta">+ أضف جدولك الأسبوعي</a>
            <button type="button" class="start-add-class" data-add-class>+ إضافة فصل</button>`;
    }
    ...
    /* ══════════════════════════════════════════════════════════════════
       مقولاتٌ في العلم والمعلّم — لبطاقة الإجازة
       ══════════════════════════════════════════════════════════════════
       طلبُ المعلّم (٣ سبتمبر ٢٠٢٦): «تحت (إجازة سعيدة) مقولاتٌ أثريّةٌ عن
       المعلّم».

       **والنسبةُ أمانة.** فما لم تثبت نسبتُه لأحدٍ يُقال «مَثَلٌ سائر» ولا
       يُلصق بعالمٍ لم يقله — والمعلّمون أعرفُ الناس بهذا، ونسبةٌ خاطئةٌ
       تُسقط ثقةَ الشاشة كلِّها. وواحدةٌ منها قِيست في المصادر عند كتابتها
       (أثرُ عليٍّ لكميل بن زياد، في نهج البلاغة).

       والاختيارُ باليوم لا بالعشوائيّ: تثبت المقولةُ طوالَ اليوم فلا تتبدّل
       كلّما رجع المعلّمُ إلى الرئيسيّة، وتتغيّر غداً. */
    const QUOTES = [
        /* الشِّعرُ شطران لا سطرٌ يلتفّ: `t2` تعني بيتاً يُكسر في موضعه. */
        { t: 'قُمْ للمعلِّمِ وَفِّهِ التبجيلا', t2: 'كادَ المعلِّمُ أن يكونَ رَسولا', by: 'أحمد شوقي' },
        { t: 'العلماءُ ورثةُ الأنبياء', by: 'حديث شريف' },
        { t: 'إنّما بُعثتُ معلِّماً', by: 'حديث شريف' },
        { t: 'إذا ماتَ الإنسانُ انقطعَ عنه عملُه إلّا من ثلاث: صدقةٍ جارية، أو عِلمٍ يُنتفَعُ به، أو ولدٍ صالحٍ يدعو له', by: 'حديث شريف' },
        { t: 'الناسُ ثلاثة: عالِمٌ ربّانيّ، ومتعلِّمٌ على سبيلِ نجاة، وهَمَجٌ رَعاع', by: 'عليّ بن أبي طالب' },
        { t: 'تعلَّموا العلمَ، وتعلَّموا للعلمِ السكينةَ والوَقار', by: 'عمر بن الخطّاب' },
        { t: 'العِلمُ ما نفَع، ليس العِلمُ ما حُفِظ', by: 'الإمام الشافعي' },
        { t: 'مَن علَّمني حرفاً صِرتُ له عبداً', by: 'مَثَلٌ سائر' },
        { t: 'العِلمُ يرفعُ بيتاً لا عِمادَ له', t2: 'والجهلُ يهدِمُ بيتَ العِزِّ والكَرَمِ', by: 'من الشِّعر السائر' },
        { t: 'مَن لم يذُقْ ذُلَّ التعلُّمِ ساعةً، تجرَّعَ ذُلَّ الجهلِ أبداً', by: 'مَثَلٌ سائر' }
    ];

    /** مقولةُ اليوم — ثابتةٌ فيه، تتبدّل بتبدّله. */
    function quoteOfDay() {
        const d = new Date();
    ...
    function restCardHtml(kind, off) {
        if (kind === 'weekend') {
            /* معاينةٌ تنتظر قرارَه (?rest=1): حرفٌ أخفُّ ومقولةُ اليوم تحته. */
            if (document.body.classList.contains('rest-quote')) {
                const q = quoteOfDay();
                return `
                    <div class="home-hero-alt rest-q">
                        <div class="rq-t">إجازة سعيدة</div>
                        <div class="rq-sub">نلقاك الأحد بإذن الله</div>
                        <div class="rq-rule" aria-hidden="true"></div>
                        <blockquote class="rq-q${q.t2 ? ' is-verse' : ''}">${
                            q.t2 ? `<span class="rq-line">${esc(q.t)}</span><span class="rq-line">${esc(q.t2)}</span>`
                                 : esc(q.t)}</blockquote>
                        <div class="rq-by">${esc(q.by)}</div>
                    </div>`;
            }
            return `
    ...
    async function render(container) {
        ...
        let body, bodyMod = '';
        if (!hasClasses) {
            // أول فتح: لافتةُ «بقيت فصولك» والزرّان — بنيةُ الحالة التالية عينُها
