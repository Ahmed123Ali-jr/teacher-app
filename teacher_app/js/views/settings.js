...
(function (global) {
    ...
    /* ══ رقمُ الإصدار — **ثلاثةُ مواضعَ تتحرّك معاً** ══
       ١) هنا (ما يراه المعلّم في «عن التطبيق»)
       ٢) `package.json` → `version`
       ٣) `ios/App/App.xcodeproj` → `MARKETING_VERSION` (في Debug و Release)
       وكان بينها فرقٌ: هنا `0.9.0` وفي Xcode `1.0` — ورقمان مختلفان يراهما
       مراجعُ آبل. فوُحّدت على `1.0.0` في ٥ سبتمبر ٢٠٢٦.
       ولا تُغيَّر واحدةٌ وحدَها. و`CURRENT_PROJECT_VERSION` **ليس منها** —
    ...
    /* **فارغٌ عمداً حتى يصل العنوانُ الحقيقيّ.** كان هنا
       ...
       ── ووصل ٦ سبتمبر ٢٠٢٦: أنشأ `fusool96@gmail.com` **مخصَّصاً للتطبيق**
       لا بريدَه الشخصيّ — فلا يُقرن اسمُه بكلّ رسالة دعمٍ من ٥٠٠٠ معلّم،
       ويُسلَّم لغيره لو سلّم الدعمَ يوماً.
       ── وإن أُفرغ ثانيةً: العنوانُ يختفي ولا يُستبدل بمزيَّف. سطرُ «الدعم»
       في «عن التطبيق» لا يُعرض، وقسمُ «الدعم الفنّي» يدلّ على «ملاحظاتكم».
       ── والموضعان يتبعان هذا السطرَ وحدَه. */
    ...
    function bindBell(container) {
        ...
        container.querySelector('#bell-notif')?.addEventListener('click', async () => {
            ...
            /* ══ والشاشةُ تُعاد بعد الجواب ══
               حالُ الإذن يُقرأ عند الرسم وحدَه. فمن ضغط «السماح» وأذن بقي
               يقرأ «الإشعاراتُ غير مسموحة، فلن يرنّ والتطبيقُ مغلق» والزرُّ
               تحتها — فيظنّ أنّ إذنَه لم يقع، ويضغط ثانيةً **ولا يُظهر iOS
               الحوارَ مرّتين**، فيخرج وهو يحسب الجرسَ معطوباً.
               (قِيس على iOS ١٣ سبتمبر ٢٠٢٦: التحذيرُ بقي حتى خرجتُ من
               الشاشة وعدت، فانقلب إلى «يرنّ والتطبيق مغلق».) */
            render(container);
        });
    }
    ...
    /* ══ سطرٌ يقول ما لا تحمله النسخة ══
       النسخةُ ملفُّ JSON فيه صفوفُ ثمانيةَ عشرَ جدولاً — ولا ملفَّ فيها:
       كتبُ المعلّم وملفّاتُ المنهج في `book_files` بجهازه (وهي ليست من
       `STORE_NAMES` فلا يمرّ عليها `exportAll`)، وصورُ الشواهد في دلو
       `evidence` بروابطَ لا ببيانات.

       وكان هذا مسكوتاً عنه، ومنطقةُ الخطر تقول فوقه «يُنصح بتصدير نسخة
       احتياطية أولاً» — فمن صدّر ثمّ مسح ثمّ استعاد وجد كتبَه فارغةً
       وشواهدَه بلا صور، وهو يظنّ أنّه احتاط.

       ولا تُحشى النسخةُ بها: قرارُه (٢١ أغسطس ٢٠٢٦) أنّ الكتب تبقى محليّةً،
       والكتابُ الواحد ‎50MB‎ — فملفٌّ بمئات الميغا لا يُنزَّل على جوّال.
       فالعلاجُ صدقٌ لا بناء. وشعارُ المدرسة خارجَ السطر بقوله (٤ سبتمبر):
       «لا نحتاج شعار المدرسة، الأهمّ البيانات الأخرى». */
    function backupBody(prefs) {
        return `
            ...
            <p class="text-muted" style="font-size:var(--fs-sm); margin: var(--space-4) 0 0; line-height:1.8;">
                يُحفظ كلُّ شيءٍ إلّا الملفّات: كتبك وتوزيع المنهج وصورَ الشواهد.
            </p>
        `;
    }
    ...
    /* شرحان قصيران بحروفه (٤ سبتمبر ٢٠٢٦): «الشرح تحت كلٍّ منهما طويلٌ
       جدّاً». وكانا يعدّان ما يُحذف عدّاً — فصولاً وطلاباً واختباراتٍ وكتباً
       وملفاتِ منهجٍ وشواهد — والقائمةُ الطويلةُ تُقرأ مسحاً لا قراءة.
       والتفصيلُ لم يضع: نافذتا التأكيد تقولانه عند الضغط، وهناك يُقرأ.
       ويبقى سطرُ النسخة الاحتياطيّة — رابطٌ يُنقذ، لا شرحٌ يُقرأ. */
    function dangerBody() {
        return `
            <div class="dz">
                <div class="dz-t">${Icons.svg('broom')} مسح البيانات</div>
                <p class="dz-h">تحذف جميع بياناتك ويبقى حسابك.</p>
            ...
            <div class="dz is-final">
                ...
                <p class="dz-h">يحذف حسابك وبياناتك بشكل نهائي.</p>
                <label class="dz-ack">
