...
(function (global) {
    ...
    async function isDone(teacher) {
        try {
            /* ثلاثُ حالاتٍ لا اثنتان. و«لم تُكتب» ليست كـ«كُتبت كذباً»:
               حين يفشل الدفعُ إلى الخادم تُكتب `false` صراحةً (أسفلَ هذا
               الملفّ) ليُعاد المعلّمُ ويضغط ثانية. ولو خُلطت بالمعدوم لسبقها
               الفحصُ القديم أدناه فوجد المدرسةَ والقسمَ مكتوبةً **محلّياً**
               فقال «مهيّأ» — فيُطرد من الشاشة التي أُعيد إليها، ولا سبيلَ
               له إلى إتمام تهيئةٍ لم تصل الخادمَ أبداً. */
            const mark = await global.TeacherDB.Settings.get('onboarded');
